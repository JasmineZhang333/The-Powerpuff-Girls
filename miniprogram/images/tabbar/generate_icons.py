...
def create_card_icon(size=81):
    ...
    img.save('card.png')
    print('Created card.png')

def create_climb_icon(size=81):
    img = Image.new('RGBA', (size, size), (0, 0, 0, 0))
    draw = ImageDraw.Draw(img)
    
    draw.line([8, 72, 40, 22, 72, 72], fill='#f5a623', width=4)
    draw.ellipse([33, 18, 47, 32], fill='#f5a623')
    draw.line([40, 32, 40, 48], fill='#888888', width=4)
    draw.line([30, 40, 40, 48, 50, 40], fill='#888888', width=4)
    draw.line([35, 36, 28, 26], fill='#888888', width=3)
    draw.line([45, 36, 52, 26], fill='#888888', width=3)
    draw.line([35, 48, 28, 58, 24, 68], fill='#888888', width=3)
    draw.line([45, 48, 52, 58, 56, 68], fill='#888888', width=3)
    draw.ellipse([18, 22, 28, 32], fill='#f5a623')
    draw.ellipse([52, 30, 62, 40], fill='#f5a623')
    draw.ellipse([28, 58, 38, 68], fill='#f5a623')
    
    img.save('climb.png')
    print('Created climb.png')

def create_profile_icon(size=81):
    img = Image.new('RGBA', (size, size), (0, 0, 0, 0))
    draw = ImageDraw.Draw(img)
    
    draw.ellipse([29, 12, 51, 34], fill='#888888')
    draw.ellipse([29, 12, 51, 34], outline='#f5a623', width=2)
    draw.ellipse([18, 34, 62, 72], fill='#888888')
